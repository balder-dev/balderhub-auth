...
class ScenarioAuthpermAuthenticated(AbstractScenarioAuthpermAuthenticated):
    """
    ...
    @balder.parametrize_by_feature(
        'resource_rule', (Client, 'has_perm_for', 'resolved_resources_without_permissions')
    )
    def test_client_has_no_perm_for_resources(self, resource_rule: ResourceRule):
        ...
        :param resource_rule: the resource rule for a resource the client has no permission for
        """
        self.Client.sm_auth.assert_expected_state(self.Client.sm_auth.State.AUTHENTICATED)
    ...
    @balder.parametrize_by_feature(
        'resource_rule', (Client, 'has_perm_for', 'resolved_resources_with_permissions')
    )
    def test_client_has_perm_for_resources(self, resource_rule: ResourceRule):
        ...
        :param resource_rule: the resource rule for a resource the client has permission for
        """
        self.Client.sm_auth.assert_expected_state(self.Client.sm_auth.State.AUTHENTICATED)
